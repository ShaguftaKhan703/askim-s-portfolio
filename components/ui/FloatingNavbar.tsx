    ...
    >
      <nav className="flex items-center justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16">
        ...
        <Link href="#home">
          ...
          >
            <span className="text-primary">abd</span>
        ...
        {/* Mobile Menu Button */}
        <button
          className="block sm:hidden text-primary text-2xl focus:outline-none"
        ...
        {isMenuOpen && (
          <motion.ul
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3 }}
            className="absolute top-16 left-0 w-full bg-background shadow-lg flex flex-col items-center py-4 sm:hidden border-b border-primary/10"
          >
            {navItems.map((item, idx) => (
              <li key={idx} className="w-full text-center py-2">
                <Link
                  href={item.link}
                  className={cn(
                    "flex items-center justify-center py-3 px-4 text-lg font-semibold space-x-2",
                    activeSection === item.link.slice(1)
                      ? "text-primary bg-primary/10 border border-primary-foreground"
                      : "text-foreground hover:text-primary"
                  )}
